...
class CanonicalDirective(Base):
    ...
    __table_args__ = (
        CheckConstraint(
            "scope IN ('global','workspace','team','private')", name="directive_scope_check"
        ),
        CheckConstraint(
            "status IN ('active','draft','archived','deprecated')", name="directive_status_check"
        ),
        {"schema": "dkb"},
    )
    ...
    canonical_meta: Mapped[dict] = mapped_column(
...
class RawToCanonicalMap(Base):
    ...
    __table_args__ = (
        ...
        CheckConstraint(
            "mapping_status IN ('candidate','accepted','rejected','manual')",
            name="mapping_status_check",
        ),
        {"schema": "dkb"},
    )
...
class DirectiveRelation(Base):
    ...
    __table_args__ = (
        ...
        CheckConstraint(
            "strength >= 0 AND strength <= 1", name="directive_relation_strength_check"
        ),
        CheckConstraint(
            "left_directive_id <> right_directive_id", name="directive_relation_not_same_check"
        ),
        {"schema": "dkb"},
    )
    ...
    relation_meta: Mapped[dict] = mapped_column(
        JSONB, nullable=False, server_default=text(jsonb_default)
    )
    created_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True), server_default=func.now(), nullable=False
