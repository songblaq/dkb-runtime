...
class Source(Base):
    ...
    metadata_json: Mapped[dict] = mapped_column(
        "metadata", JSONB, nullable=False, server_default=text(jsonb_default)
    )
...
class SourceSnapshot(Base):
    ...
    capture_status: Mapped[str] = mapped_column(
        Text, nullable=False, server_default=text("'captured'")
    )
    snapshot_meta: Mapped[dict] = mapped_column(
