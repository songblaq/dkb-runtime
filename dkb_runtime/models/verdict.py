...
class Verdict(Base):
    ...
    __table_args__ = (
        CheckConstraint("provenance_state IN ('official','vendor','community','individual','unknown')", name="verdict_provenance_state_check"),
        CheckConstraint("trust_state IN ('unknown','reviewing','verified','caution','blocked')", name="verdict_trust_state_check"),
        CheckConstraint("legal_state IN ('clear','custom','no_license','removed','restricted')", name="verdict_legal_state_check"),
        CheckConstraint("lifecycle_state IN ('active','stale','dormant','archived','disappeared')", name="verdict_lifecycle_state_check"),
        CheckConstraint("recommendation_state IN ('candidate','preferred','merged','excluded','deprecated')", name="verdict_recommendation_state_check"),
        {"schema": "dkb"},
    )
    ...
    dimension_model_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True), ForeignKey("dkb.dimension_model.dimension_model_id", ondelete="CASCADE"), nullable=False
    )
...
class Pack(Base):
    __tablename__ = "pack"
    __table_args__ = (
        CheckConstraint("pack_type IN ('safe','lean','starter','role','experimental','custom')", name="pack_type_check"),
